...
export default function TrustSignals() {
  return (
    <section className="section py-5" style={{
      ...
      borderBottom: '1px solid var(--ci-divider)',
    }}>
        ...
        <div className="columns is-mobile is-multiline has-text-centered">
          {stats.map(stat => (
            <div key={stat.label} className="column is-6-mobile is-3-tablet">
              <p style={{ fontSize: '2rem', fontWeight: 800, color: 'var(--ci-text)', lineHeight: 1 }}>{stat.value}</p>
              <p style={{ fontSize: '0.8rem', color: 'var(--ci-text-muted)', marginTop: '0.35rem', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{stat.label}</p>
            </div>
          ))}
        </div>
      </div>
